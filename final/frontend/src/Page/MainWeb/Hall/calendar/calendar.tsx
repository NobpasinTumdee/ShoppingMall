...
const Carlendar: React.FC = () => {
    ...
    const cellRender = (date: Dayjs) => {
        // กรองการจองที่ยืนยันแล้ว (Status === true)
        const confirmedBookings = bookings.filter((booking) => booking.Status);

        // กรองการจองที่เริ่มในวันที่ปัจจุบัน
        const dayStartBookings = confirmedBookings.filter((booking) =>
            dayjs(booking.StartDateTime).isSame(date, "day")
        );

        // กรองการจองที่สิ้นสุดในวันที่ปัจจุบัน
        const dayEndBookings = confirmedBookings.filter((booking) =>
            dayjs(booking.EndDateTime).isSame(date, "day")
        );

        // กรองการจองที่ครอบคลุมวันนั้นในช่วงเวลา
        const dayInRangeBookings = confirmedBookings.filter((booking) =>
