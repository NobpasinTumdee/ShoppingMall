...
const ServicePage: React.FC = () => {
  ...
  // ฟังก์ชันตรวจสอบข้อมูลก่อนการส่ง
  const validateForm = () => {
    const newErrors: any = {};
    ...
    // ตรวจสอบ StatusService
    if (!formData.StatusService || formData.StatusService === "") {
      newErrors.StatusService = "Status Service is required.";
    }
  ...
  const LocationStr = useMemo(() => {
    if (!Location) return "";
    return `Zone ${Location?.ProductType?.NameType || ""} Area ${
      Location?.ProductType?.NameType || ""
    }${Location?.ProductType?.ID || ""}${Location?.ID || ""}`;
  }, [Location]);
  //
  ...
  const handleSubmitEdit = async (e: any) => {
    ...
    if (Object.keys(validationErrors).length > 0) {
      setErrors(validationErrors); // เก็บ errors
      message.error("Please fill out all required fields."); // แสดงข้อความ error
      return; // ถ้ามี error ให้หยุดการส่งข้อมูล
    }
    // ถ้าไม่มี error ก็สามารถส่งข้อมูลได้
    await CreateServicefuc(formData);
  };
  const CreateServicefuc = async (formData: any) => {
    ...
    if (formData.StatusService === "pending") {
      ...
      try {
        const res = await UpdateStoreByid(
          String(formData.StoreID),
          valuesStoreUpdate
        );
        if (res.status === 200) {
  ...
  const columns: TableColumnsType<ServiceInterface> = [
    ...
    },
    {
      ...
      render: (_, data) => (
        <a
          onClick={() =>
            StateService(String(data.ID), String(data.StatusService))
          }
          style={{ color: "#a78f48" }}
        >
          Edit
  ...
  const handleSubmitEquipment = async (e: any) => {
    e.preventDefault();
    // ตรวจสอบว่ามีการเลือกข้อมูลใน Select หรือไม่
    if (!formData2.InventoryID || !formData2.Quantity) {
  ...
  const ReturnEquipment = async (data: EquipmentInterface) => {
    if (
      data.Inventory?.QuantityInventory &&
      data.Quantity &&
          ...
          if (res.status === 200) {
            ...
            try {
              ...
              if (res.status === 200) {
                ...
                //fetch ข้อมูล Equipment อีกรอบ
                const res = await EquipmentByServiceID(
  ...
  const ServiceCompleted = async () => {
    ...
    try {
      const selectedService = Service.find(
        (Service) => Service.ID === formData2.ServiceID
      );
      ...
      <div className="serviceanimation">
        ...
        <div className="formService">
          <form onSubmit={handleSubmitEdit}>
            ...
            <p style={{ color: "#bea66a" }}>
              {LocationStr || "-- Please select Store --"}
            </p>
          ...
          {/* {formData.StoreID}{formData.UserID}{formData.Location}{formData.Description}{formData.StatusService} */}
        </div>
        <div style={{ margin: "0 0 15%" }}>
          <h2 style={{ margin: "0 20%", fontFamily: '"Abel", sans-serif' }}>
      ...
      {Popup && (
        <>
          <div className="PopupService">
            <h1 style={{ margin: "0 0 20px", textAlign: "center" }}>
              Request Equipment
            </h1>
            <form onSubmit={handleSubmitEquipment}>
              ...
              <button
                style={{ padding: "5px 15px", margin: "0 5px" }}
            ...
            <div style={{ margin: "5px 0" }}>
              InventoryID : {formData2.InventoryID} Quantity :{" "}
              {formData2.Quantity} ServiceID : {formData2.ServiceID}
            </div>
            <div
              className="Equipment"
              style={{
                backgroundColor: "#bea66a",
                color: "#fff",
                fontWeight: "900",
              ...
              {Equipment.map((data, index) => (
                <>
                  <div key={index} className="Equipment">
                    ...
                    <p style={{ height: "30px", overflowY: "scroll" }}>
                      {data.Inventory?.InventoryName}
                    </p>
                    <p>
                      {data.DateEquipment
                        ? new Intl.DateTimeFormat("en-GB").format(
                            new Date(data.DateEquipment)
                          )
                        : "No Date"}
                    </p>
                    <p>{data.Quantity}</p>
                    <button
                      onClick={() => ReturnEquipment(data)}
                      style={{
                        backgroundColor: "#fff",
                        borderRadius: "20px",
                        height: "35px",
