    ...
    <>
      ...
      >
        ...
        <div>
          <Row justify="center" style={{ marginTop: "30px" }}>
            <Col span={24}>
              <Card title={`History for Card ID ${cardID}`} bordered={false} style={{ marginBottom: 10}}>
                <Row>
                  <Col span={12}>
                  <p style={{ marginTop: 10, lineHeight: 0.7}}>Card Type: {card?.TypePark?.Type || " "}</p>
                    <p style={{lineHeight: 0.7}}>
                      Status:{" "}
                      {card?.StatusCard?.Status === "IN"
                        ? "IN"
                        : card?.StatusCard?.Status === "OUT"
                        ? "OUT"
                        : "Default"}
                    </p>
                    <p style={{lineHeight: 0.7}}>Expiry Date: {formatDate(card?.ExpiryDate || "")}</p>
                  </Col>
                </Row>
                <Table
                  columns={columns}
