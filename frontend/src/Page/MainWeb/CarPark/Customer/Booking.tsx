...
const CustomerParkingBooking: React.FC = () => {
  ...
  const handleSubmit = () => {
    if (!licensePlate || !carColor || selectedZoneIndex === null) {
      message.error("Please complete all fields and select a zone.");
      return;
    }

    const selectedZone = zones[selectedZoneIndex];
    const selectedCard =
      selectedZoneIndex !== null ? cards[selectedZoneIndex] : null;

    if (
      selectedCard?.StatusCard?.Status === "ON" ||
      selectedCard?.StatusCard?.Status === "Expire" ||
      selectedCard?.StatusCard?.Status === "Un Used"
    ) {
      message.error("This card is not available for booking.");
      return;
    }

    if (!isMember) {
      setIsModalRequiredVisible(true);
      return;
    }
    ...
    message.success("Parking booked successfully!");
  };

  // Add the missing handleMembershipCancel function
  const handleMembershipCancel = () => {
    setIsModalMembershipVisible(false);
  };

  // Add the missing handleRequiredCancel function (for the membership modal)
  const handleRequiredCancel = () => {
    setIsModalRequiredVisible(false);
  };

  // Add missing handlers
  const handleCardModalOk = () => {
    // You can implement logic for the "Ok" button in the ID Card Verification modal here
    if (cardInput.length === 4 && !isNaN(Number(cardInput))) {
      setIsModalCardVisible(false);
      message.success("ID card verified!");
    } else {
  ...
  const handleRequiredOk = () => {
    // Implement the logic for the "Ok" button in the Membership Required modal
    setIsModalRequiredVisible(false);
    setIsModalMembershipVisible(true); // Open the membership registration modal
  };

  const handleCardModalCancel = () => {
    // Handle cancel for the ID Card Verification modal
    setIsModalCardVisible(false);
  };
